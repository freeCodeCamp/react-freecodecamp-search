...
function renderResults(reset, results) {
  return results.map(result => (
    <li
      className='fcc_resultItem list-group-item'
      key={`${result._id}`}
      >
      <a
      ...
      <Row className='fcc_resultHeading-wrap'>
        <Col className='fcc_resultHeading' xs={12}>
          <h4 className="list-group-item-heading">{result._index.replace(/s$/, '').toUpperCase()}</h4>
        </Col>
      </Row>
      <Row className='fcc_resultContent'>
        <Col className='fcc_resultTitle' xs={4}>
          <h5><strong>{result._source.title}</strong></h5>
        </Col>
